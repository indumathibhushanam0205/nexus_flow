    ...
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http
            ...
            .authorizeHttpRequests { auth ->
                auth.requestMatchers("/auth/**", "/swagger-ui/**", "/v3/api-docs/**").permitAll() // Allows all requests without login for mwntioned there
                auth.anyRequest().authenticated()  //Require a token for everything else
            }
            // Add our custom JWT filter BEFORE the standard login process
            .addFilterBefore(jwtAuthFilter , UsernamePasswordAuthenticationFilter::class.java)

        return http.build()
    }

    //password encoder to avoid password visibility in database
    @Bean
